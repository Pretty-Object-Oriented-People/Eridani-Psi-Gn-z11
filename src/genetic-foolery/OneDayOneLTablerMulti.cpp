...
struct DayTimetable {
	...
	double cancerocity() const {
		double cancerocity = 0;
		FOR_Group(g){
			for(HourOfDay h = 0; h < NUM_Hours-2; h++) if(GroupHour2Subject[g][h] != Subject_None && GroupHour2Subject[g][h+1] == Subject_None && GroupHour2Subject[g][h+2] != Subject_None) cancerocity += 10;
			for(HourOfDay h = 0; h < NUM_Hours-1; h++) if(GroupHour2Subject[g][h] != Subject_None) if(GroupHour2Subject[g][h] == GroupHour2Subject[g][h+1] && GroupHour2Room[g][h] != GroupHour2Room[g][h+1]) cancerocity += 20;
		}
		return cancerocity;
	}
...
struct IntermediateCost {
	...
	json_object* toJSON() const{
		let json = json_object_new_object();
		json_object_object_add(json, "Avg Day Length", json_object_new_double(avgDayLength));
		json_object_object_add(json, "Cancerocity", json_object_new_double(cancerocity));
		return json;
	}
...
bool eval_solution(const DayTimetable& timetable, IntermediateCost &c){
	...
	c.cancerocity = timetable.cancerocity();
	return true;
}
...
std::vector<double> calculate_MO_objectives(const GA_Type::thisChromosomeType &X){
	return {
		X.middle_costs.avgDayLength,
		X.middle_costs.cancerocity
	};
}

json_object* odolt_gens_arr;

void MO_report_generation(int generation_number, const EA::GenerationType<DayTimetable, IntermediateCost> &last_generation, const std::vector<unsigned int>& pareto_front){
	...
	/*if(odolt_gens_arr){
		let json = json_object_new_object();
		json_object_object_add(json, "best_genes", best_genes.toJSON());
		// json_object_object_add(json, "best_stats", last_generation.)
		json_object_array_add(odolt_gens_arr, json);
	}*/
}
...
void save_results(const GA_Type &ga_obj){
	...
	let json = json_object_new_object();
	json_object_object_add(json, "gens", odolt_gens_arr);
	...
	json_object_to_file("odoltablerm-report.json", json);
...
int OneDayOneLTablerMultiRun(){
	...
	odolt_gens_arr = json_object_new_array();
